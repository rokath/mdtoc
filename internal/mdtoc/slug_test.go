...
// TestCrossnoteSluggerExamples verifies the Crossnote / Markdown Preview
// Enhanced heading-ID behavior used for slug=crossnote.
func TestCrossnoteSluggerExamples(t *testing.T) {
	slugger := NewCrossnoteSlugger()
	cases := []struct{ input, want string }{
		{"1. Intro", "1-intro"},
		{"1.1. API", "11-api"},
		...
		{"🚀 !!!", "-"},
		{"foo0 ~  bar", "foo0---bar"},
		{"An ATX title with closing hash markers  ####", "an-atx-title-with-closing-hash-markers--"},
	}
	for _, tc := range cases {
		if got := slugger.Next(tc.input); got != tc.want {
			t.Fatalf("crossnote slugger.Next(%q) = %q, want %q", tc.input, got, tc.want)
		}
	}
}
